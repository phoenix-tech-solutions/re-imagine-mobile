      ...
      <View className="p-6">
        ...
        <View className="gap-y-4">
          {projects.map((project) => (
            <TouchableOpacity 
              key={project.id}
              onPress={() => navigation.push('ProjectDetails', { projectId: project.id })}
            ...
            >
              <View className="flex-row items-center justify-between mb-2">
                <View className="flex-row items-center">
                  ...
                  />
                  <Text style={{ color: getStatusColor(project.status) }} className="ml-2 font-bold capitalize">
                    {project.status.replace('_', ' ')}
                  </Text>
                </View>
                <View className="flex-row items-center">
                  <MaterialIcons name="star" size={16} color={getDifficultyColor(project.difficulty_level)} />
                  <Text style={{ color: getDifficultyColor(project.difficulty_level) }} className="ml-1 capitalize">
                    {project.difficulty_level}
                  </Text>
                </View>
              </View>
              <Text className="text-lg font-bold mb-2">{project.title}</Text>
              <Text className="text-gray-500 mb-4">{project.description}</Text>
              
              <View className="mb-3">
                <Text className="font-bold mb-2">Skills Required:</Text>
                <View className="flex-row flex-wrap">
                  {project.required_skills.map((skill, index) => (
                    <View key={index} className="bg-blue-100 px-2 py-1 rounded-md mr-2 mb-1">
                      <Text className="text-xs text-blue-800">{skill}</Text>
                    </View>
                  ))}
                </View>
              </View>
              ...
              <View className="flex-row items-center justify-between w-full mt-4">
                ...
                <TouchableOpacity 
                  ...
                  <Text className={`${project.member_count === project.max_team_size ? 'text-gray-500' : 'text-white'}`}>
